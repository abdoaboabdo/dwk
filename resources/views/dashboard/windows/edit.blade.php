            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="box box-primary" style="overflow: hidden">
                        ...
                        <form role="form" action="{{route('dashboard.windows.update',$window->id)}}" method="post" enctype="multipart/form-data">
                            @csrf
                            {{method_field('put')}}
                            <div class="box-body">
                                <div class="row " style="position: relative;">
                                    <div class="col-md-4">
                                        <div class="form-group has-success">
                                            <label for="category_id" >نوع النافذة</label>
                                            <select name="category_id" id="category_id" class="form-control form-control " style="padding: 0 12px">
                                                ...
                                                @foreach($windowCategory->children as $child)
                                                    <option value="{{$child->id}}" {{$window->category_id === $child->id ? 'selected' : ''}}>{{$child->name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group has-success">
                                            ...
                                            <select name="color" id="color" class="form-control">
                                                ...
                                                @foreach ($color as $c)
                                                    <option value="{{$c->name}}" {{ $window->color === $c->name ? 'selected' : '' }}>{{$c->name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group has-success">
                                            ...
                                            <select name="thickness" id="thickness" class="form-control form-control " style="padding: 0 12px">
                                                ...
                                                <option value="1.3" {{$window->thickness === 1.3 ? 'selected' : ''}}>1.3</option>
                                                <option value="1.5" {{$window->thickness === 1.5 ? 'selected' : ''}}>1.5</option>
                                                <option value="1.8" {{$window->thickness === 1.8 ? 'selected' : ''}}>1.8</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group {{$window->aluminum_type_id !== null ? 'has-success' : ''}}">
                                            ...
                                            <select name="aluminum_type_id" id="aluminum_type_id" class="form-control form-control " style="padding: 0 12px">
                                                ...
                                                @foreach ($aluminum_types as $aluminum_type)
                                                    <option value="{{$aluminum_type->id}}" {{$window->aluminum_type_id === $aluminum_type->id ? 'selected' : ''}}>{{$aluminum_type->name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group has-success">
                                            ...
                                            <input type="text" name="price" id="price" value="{{$window->price}}" class="form-control " placeholder="ادخل السعر">
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-4 ">
                                        <div class="form-group has-success">
                                            ...
                                            <textarea name="description" id="description" class="form-control "  cols="30" rows="10" placeholder="ادخل الوصف">{{$window->description}}</textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                ...
                                <div class="row ">
                                    ...
                                    @foreach ($window->images as $key=>$image)
        ...
        $(document).ready(function() {// imgInp
            ...
            function validation(){
                if ($('#category_id').val() === ''){
                    ...
                    if (validateObject.color&&validateObject.thickness&&validateObject.price && validateObject.description && validateObject.filename && validateObject.aluminum_type_id){
                                        ...
                                        me.parent('.file').next().children('.load').fadeIn(function () {
                                            meme.parent('.file').next().children('.load').fadeOut(function () {
                                                if(id !== ''){
                                                    ...
                                                    let window_id='{{$window->id}}';
                                                    let data = {'_token':$('meta[name=csrf-token]').attr('content'),window_id,'image':dataImg};
                                                    $.ajax({
                                                        url:url,
                                                        method:'post',
                                                        data:data,
            ...
            $('#aluminum_type_id').change(function (e) {
                if (e.target.value === ''){
                    $('#aluminum_type_id').parents('.form-group').removeClass('has-success').addClass('has-error');
                    validateObject.aluminum_type_id=false;
                    validation();
                }else {
                    $('#aluminum_type_id').parents('.form-group').removeClass('has-error').addClass('has-success');
                    validateObject.aluminum_type_id=true;
                    validation();
                }
            });
